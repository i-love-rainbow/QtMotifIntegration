...
Boolean qmotif_event_dispatcher(XEvent *event)
{
    ...
    Boolean r = static_d->dispatchers[event->xany.display][event->type](event);
    // ACR00312733, explicitly sent a focusin event after mouse button(only left button has function in RSDebugger)
    // release, so that the cursor get repaint.
    if (r && event->type == ButtonRelease)
    {
        event->type = XFocusIn;
        r = static_d->dispatchers[event->xany.display][event->type](event);
    }
    return r;
}
